...
// Animation delay scaled by the user-selected speed factor (1x/2x/4x)
function animDelay(ms) {
    var s = (typeof G.getSpeedFactor === 'function') ? G.getSpeedFactor() : 1;
...
function processNext() {
    ...
    switch (action.type) {
        case 'move':
            ...
            d.col = action.col;
            G.drawGame();
            G.delay(animDelay(300), processNext);
            break;

        case 'turn':
            d.dir = action.dir;
            G.drawGame();
            G.delay(animDelay(200), processNext);
            break;
        ...
        case 'get':
            ...
            G.drawGame();
            G.delay(animDelay(500), processNext);
            break;

        case 'stage_clear':
            showOverlay(true);
            G.log('--- 關卡完成！準備進入下一關 ---');
            G.delay(animDelay(1200), function() {
                showOverlay(false);
                advanceToNextStage();
                G.setAnimating(false);
                enableRunButton(true);
