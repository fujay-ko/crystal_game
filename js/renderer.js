...
G.drawGame = function(overrideMsg) {
    ...
    // HiDPI：依 devicePixelRatio 放大底層解析度，避免高解析螢幕模糊
    var dpr = window.devicePixelRatio || 1;
    var vw = C.CANVAS_WIDTH, vh = C.CANVAS_HEIGHT;
    ...
    // 3b. Target highlight: ring around the current stage's target crystal
    //     (first remaining crystal), so students can see where to go
    if (d.crystals_on_map.length > 0) {
        var target = d.crystals_on_map[0];
        var tx = target.col * tile + tile / 2;
        var ty = target.row * tile + tile / 2;
        ctx.save();
        ...
        ctx.shadowBlur = 18;
        ctx.beginPath();
        ctx.arc(tx, ty, tile / 2 - 4, 0, Math.PI * 2);
        ctx.stroke();
        ctx.restore();
    }
    ...
    ctx.font = "20px 'Comic Sans MS', Arial";
    ctx.fillText('任務：收集水晶', L.PROGRESS_TITLE.x, L.PROGRESS_TITLE.y);
